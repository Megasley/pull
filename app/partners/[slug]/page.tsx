...
export default async function PartnerOrgHubPage({
  ...
  // Suggested opportunities (real projects/issues matched on the org's
  // admin-configured skills + the participant's own detected skills) always
  // show, alongside curated ones rather than only as a fallback when there
  // are none — a fresh org should never show "empty," and an org with
  // curated content still benefits from a wider "browse more" section.
  const orgSkills = await listOrgSkills(org.id);
  const suggestedOpportunities = getSuggestedOpportunities([
    ...new Set([...orgSkills, ...userSkills]),
  ]);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 pt-12 pb-24 sm:px-6 lg:px-8">
      ...
      <div className="border-b border-border pb-10">
        <span className="inline-block border border-ink bg-signal px-2 py-1 font-mono text-[11px] tracking-wide text-signal-foreground uppercase">
          {org.name}
        </span>
        ...
        <div className="mt-8 border border-ink bg-signal/10 px-5 py-4">
          ...
          {(() => {
            ...
            return steps.map((step, i) => (
              ...
              >
                ...
                {i === currentIndex ? (
                  <span className="ml-auto font-mono text-[10px] tracking-wide text-ink uppercase">
                    ← you are here
                  </span>
                ) : null}
              </div>
      ...
      <div className="mt-12">
        ...
        <div className="mt-4 border border-ink bg-signal/10 px-6 py-6">
      ...
      <OpportunityExploreTracker orgSlug={slug}>
        {curatedOpportunities.length === 0 && suggestedOpportunities.length === 0 ? (
          <div className="mt-10">
            <h2 className="font-mono text-xs tracking-wide text-muted-foreground uppercase">
          ...
          <>
            {curatedOpportunities.length > 0 ? (
              <div className="mt-10">
                <h2 className="font-mono text-xs tracking-wide text-muted-foreground uppercase">
                  Curated Opportunities // {curatedOpportunities.length}
                </h2>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {opp.isPinned && (
                              <Badge className="border-ink bg-signal text-[10px] text-signal-foreground">Pinned</Badge>
                            )}
                            <Badge
                              variant="outline"
                              className={`text-[10px] ${DIFFICULTY_CLASSES[opp.difficulty] ?? ""}`}
                            >
            ...
            {suggestedOpportunities.length > 0 ? (
              <div className="mt-10">
                <h2 className="font-mono text-xs tracking-wide text-muted-foreground uppercase">
                  Suggested Opportunities // {suggestedOpportunities.length}
                </h2>
