...
## Held HTLCs lock liquidity

While an HTLC is outstanding, that slice of **outbound** balance cannot be used elsewhere on that channel. This ties directly to channel liquidity: a node can look healthy on total capacity but fail to forward because outbound is tied up in pending HTLCs. Long-held HTLCs (slow peers, attacks) are an operational risk. Monitor pending HTLCs on production nodes.
