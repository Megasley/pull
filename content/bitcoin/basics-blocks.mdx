...
<Mermaid
  caption="Headers commit to transactions and previous history; PoW makes rewriting expensive."
  chart="flowchart TD
    TxA[Tx A] --> M0[Hash]
    TxB[Tx B] --> M1[Hash]
    M0 --> Root[Merkle root]
...
<Callout type="info" title="0-conf is a business risk">
  Accepting unconfirmed transactions can be fine for tiny amounts with controls, but it
