---
title: P2P Adversarial Thinking
description: >-
  Eclipse attacks, topology leaks, compact block relay, and why Bitcoin’s gossip
  layer is a security surface.
difficulty: advanced
duration: 6 days
objectives:
  - Explain eclipse and partitioning risks for Bitcoin peers
  - Describe what compact blocks optimize and what they do not secure
  - List practical defenses for node operators and wallet backends
resources:
  - title: Eclipse Attacks on Bitcoin’s Peer-to-Peer Network
    url: "https://eprint.iacr.org/2015/263.pdf"
    kind: article
    required: true
  - title: BIP-152 Compact Block Relay
    url: "https://github.com/bitcoin/bips/blob/master/bip-0152.mediawiki"
    kind: bip
    required: true
  - title: Compact Blocks FAQ
    url: "https://bitcoincore.org/en/2016/06/07/compact-blocks-faq/"
    kind: docs
    required: true
  - title: Mastering Bitcoin
    url: "https://github.com/bitcoinbook/bitcoinbook/blob/develop/ch10_network.adoc"
    kind: book
    chapter: "Ch. 10: The Bitcoin Network"
reflectionPrompts:
  - >-
    How can an adversary isolate a node from honest peers, and what symptoms
    would you look for in logs?
  - >-
    What problem do compact blocks solve, and which attack classes remain even
    with BIP-152?
  - >-
    If you run infrastructure for wallets, which P2P settings and monitoring
    would you harden first?
lab:
  title: Node exposure worksheet
  description: >-
    Complete Steps 2–3. Document your bitcoind (or Polar) peer settings —
    maxconnections, addnode/connect, onion vs clearnet — and write a short
    adversarial threat note for that setup.
  evidence:
    - Export or screenshot of peer/connection config
    - Half-page threat note covering eclipse and DoS
    - One concrete hardening change you would make
searchQueries:
  - BIP152
---

Bitcoin’s **P2P network** is not just plumbing. Peer selection, relay, and topology affect censorship resistance, privacy, and whether your node sees the same chain tip as the honest majority.

Complete **Step 1 // Required reading** in the study plan above, then continue here.

## Step 2 — Core idea: eclipse, partitions, compact relay

Wallets and services that trust a small set of peers inherit those peers’ view of the mempool and chain. Protocol builders who ignore gossip security ship fragile products.

An **eclipse attack** aims to surround a victim with attacker-controlled peers so the victim’s view of the network is filtered. Related risks include BGP-level routing interference and intentional network partitions.

**BIP-152 compact blocks** reduce bandwidth when relaying new blocks by sending short transaction IDs peers likely already have. This improves propagation — it is **not** a substitute for validating consensus rules.

<Mermaid
  caption="Honest nodes need diverse peers; compacted relay speeds block gossip."
  chart="flowchart LR
    Miner[Miner / proposer] --> Relay[Block relay]
    Relay --> Compact[Compact block]
    Compact --> Peers[Diverse peers]
    Peers --> Validate[Full validation]"
/>

### Operator habits

- Prefer diversity (ASNs, networks, onion where appropriate)
- Monitor peer churn and unexpected tip disagreements
- Separate listening services from keys when possible

**Done when:** You can name one eclipse symptom and one thing BIP-152 does _not_ fix.

## Step 3 — Threat note for your setup (lab)

Document your node’s peer settings (`maxconnections`, `addnode`/`connect`, onion vs clearnet). Write a short threat note covering eclipse and DoS for **that** setup, plus one concrete hardening change you would make.

```bash
bitcoin-cli -regtest getnetworkinfo
# Also note bitcoin.conf / Polar peer settings you actually use
```

### Common mistakes

- Running a “full node” with one trusted outbound peer forever
- Assuming private RPC + public P2P needs no hardening
- Confusing relay policy DoS mitigations with consensus safety

**Done when:** Lab evidence (config export + threat note + one hardening change) is complete.

### Next lesson

**Security Models & Light Clients** — full validation vs SPV, filters, and AssumeUTXO tradeoffs.
