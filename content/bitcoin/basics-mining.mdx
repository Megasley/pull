...
Complete **Step 1 // Required reading** in the study plan above, then continue here. This closes **Protocol Depth**. Soft-fork _activation_ was the previous lesson — here focus on PoW and enforcement.
...
<Mermaid
  caption="Miners search for a header hash below the target; winners earn rewards."
  chart="flowchart LR
...
<Callout type="info" title="Nodes enforce rules">
  Hashrate does not rewrite consensus by itself. Invalid blocks are rejected by
  validating peers even if a miner produced them.
</Callout>
